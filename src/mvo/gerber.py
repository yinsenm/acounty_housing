...
if __name__ == "__main__":
    ...
    end_date = "2020-01-01"
    nassets = 4
    file_path = "../data/BloombergMonthly.csv"  # "./data/BloombergMonthly.csv"
    rets_df = pd.read_csv(file_path, parse_dates=['Date']).\
        query("Date >= '%s' and Date <= '%s'" % (bgn_date, end_date)).set_index(["Date"]).iloc[:, 0: nassets]
    rets = rets_df.values
